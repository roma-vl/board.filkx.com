  ...
  <AuthenticatedLayout>
    <header class="bg-white border-b shadow-sm">
      <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
          ...
          <div class="flex-shrink-0">
            <a href="/" class="text-xl font-bold text-indigo-600">{{ $t('home.logo') }}</a>
          </div>
          ...
          <nav class="hidden md:flex space-x-6">
            ...
            <a href="/ads" class="text-gray-700 hover:text-indigo-600 font-medium">{{
              $t('nav.ads')
            }}</a>
            <a href="/about" class="text-gray-700 hover:text-indigo-600 font-medium">{{
              $t('nav.about')
            }}</a>
            <a href="/contact" class="text-gray-700 hover:text-indigo-600 font-medium">{{
              $t('nav.contact')
            }}</a>
          </nav>
          ...
          <!-- Mobile menu button -->
          <div class="md:hidden">
            <button class="text-gray-700 focus:outline-none" @click="menuOpen = !menuOpen">
              <svg
      ...
      <div v-if="menuOpen" class="md:hidden px-4 pb-4 space-y-2">
        ...
        <a href="/ads" class="block text-gray-700 hover:text-indigo-600 font-medium">{{
          $t('nav.ads')
        }}</a>
        <a href="/about" class="block text-gray-700 hover:text-indigo-600 font-medium">{{
          $t('nav.about')
        }}</a>
